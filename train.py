...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default='yolov9-e-converted_2.pt', help='权重初始化的路径,在这里面设置权重的地址')
    parser.add_argument('--cfg', type=str, default='yolov9-e.yaml',
                        help='模型的配置文件,我们可以根据自己需求配置yaml文件')
    parser.add_argument('--data', type=str, default=ROOT / r'datasets/my_data.yaml', help='大家的数据集yaml文件的地址')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-high.yaml',
                        help='用于指定超参数文件,大家根据这个地址就能够找到一个yaml文件里面配置了各种超参数')
    parser.add_argument('--epochs', type=int, default=15, help='训练的轮次')
    parser.add_argument('--batch-size', type=int, default=1, help='训练的批次大小,决定了一次向模型里输入多少张图片')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640,
                        help='输入模型里的图像尺寸,YOLO系列默认都是640')
    parser.add_argument('--rect', action='store_true',
                        help='矩形训练或验证。矩形训练或验证是一种数据处理技术，其中在训练或验证过程中，输入数据会被调整为具有相同宽高比的矩形形状')
    parser.add_argument('--resume', nargs='?', const=True, default=False,
                        help='这个如果设置为True,那么会根据你最新一次没有训练完成的权重进行继续完成训练')
    parser.add_argument('--nosave', action='store_true',
                        help='只保存之后一次训练的权重文件地址,即我们得到的那个权重,默认是这样！')
    parser.add_argument('--noval', action='store_true',
                        help='当设置时，只在最后一个周期(epoch)进行验证，而不是在每个周期都进行')
    ...
    parser.add_argument('--device', default='0',
                        help='cuda device, i.e. 0 or 0,1,2,3 or cpu, 这个不在多说了,我们的GPU设备号')
    parser.add_argument('--multi-scale', action='store_true', help='当设置时，图像大小会在训练过程中变化，上下浮动50%')
    parser.add_argument('--single-cls', action='store_true',
                        help='当设置时，将多类别数据作为单类别数据训练,一般都设置为False')
    ...
    parser.add_argument('--workers', type=int, default=0,
                        help='工作的线程,Linux系统可以设置，Windows系统必须设置0否则会导致各种问题')
    ...
    parser.add_argument('--exist-ok', action='store_true',
                        help='当设置时，如果项目/名称已存在则不会报错，不会自动增加编号')
    ...
    parser.add_argument('--freeze', nargs='+', type=int, default=[0],
                        help='用于在神经网络训练过程中“冻结”（freeze）一部分层。冻结的意思是在训练过程中保持这些层的权重不变，不对它们进行更新.')
    parser.add_argument('--save-period', type=int, default=-1, help='每隔x个周期保存一次检查点，如果小于1则禁用')
    parser.add_argument('--seed', type=int, default=0,
                        help='训练的随机数种子,计算机其实是没有随机数的，都是根据随机数种子来演变出来的随机数')
    ...
    parser.add_argument('--close-mosaic', type=int, default=0,
                        help='关闭mosaic数据增强的参数，比如设置0则不关闭mosaic数据增强如果设置10则在训练后10轮关闭mosaic增强')
