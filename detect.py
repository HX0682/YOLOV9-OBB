...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp5/weights/best.pt', help='模型权重的路径或Triton URL。')
    parser.add_argument('--source', type=str, default=ROOT / 'E:\.9.23\image_big',
                        help='输入数据的来源，可以是文件、目录、URL、glob模式、屏幕捕获或网络摄像头。默认为ROOT/data/images。')
    parser.add_argument('--data', type=str, default=ROOT / 'datasets/my_data.yaml',
                        help='数据集配置文件的路径，例如coco128.yaml。默认为ROOT/data/coco128.yaml。')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640],
                        help='推理时的图像大小（高度、宽度）。默认为[640]。')
    parser.add_argument('--conf-thres', type=float, default=0.2,
                        help='检测的置信度阈值。默认为0.25,就是我们判断一个物体概率为0-1,如果大于0.25则判断是该类别')
    ...
    parser.add_argument('--device', default='0', help='指定运行设备，例如CUDA设备（0或0,1,2,3）或CPU。')
